...
from discobolo.config.config import MONTH, RECURRENTES_MAIN, RECURRENTES_REPORT


def run_recurrentes_update():
    ...
    df = df.pivot_table(
        index="NOMBRE", columns="Category", values="DESC CONCEPTO", aggfunc="first"
    ).reset_index()
    ...
    with pd.ExcelWriter(
        RECURRENTES_MAIN, engine="openpyxl", mode="a", if_sheet_exists="replace"
    ) as writer:
        ...
        header_fill = PatternFill(
            start_color="4DA6D2", end_color="4DA6D2", fill_type="solid"
        )  # Excel-style blue
